...
RES=outputs_full_plan_id/guid025_3seed.txt
echo "seed, FID, Density, Coverage (energy-guidance g0.25, align g16, no churn)" > "$RES"
for s in 42 123 7; do
  out=$("$PY" sample_eval.py --out_dir outputs_full_plan_id --decoder rect --align --grid 16 \
        --inception --metric_cpu --energy_ckpt outputs_full_plan_id/energy_critic.pt \
        --guidance 0.25 --guide_from 0.3 --n_eval 600 --save 0 --seed $s 2>&1)
